  ...
  <div class="page-container">
        ...
        <form @submit.prevent="save">
            <div class="form-grid">
                <div class="form-group full-width">
                    <label>Tên Size <span class="required">*</span></label>
                    <input v-model="form.name" placeholder="VD: M, L, 40, 41" required class="input-field" />
                </div>
                <div class="form-group full-width">
                    <label>Mô tả</label>
                    <textarea v-model="form.description" placeholder="Mô tả chi tiết" class="input-field" rows="3"></textarea>
                </div>
            </div>
    ...
    <div v-if="deleteConfirmId" class="modal-overlay" @click.self="deleteConfirmId = null">
      <div class="confirm-dialog">
        ...
        <h3>Xác nhận xóa</h3>
        <p>Bạn có chắc chắn muốn xóa Size này? Hành động này không thể hoàn tác.</p>
...
export default {
  ...
  methods: {
    ...
    async executeDelete() {
      ...
      try {
        await SizesService.delete(this.deleteConfirmId);
        await this.loadData();
