  ...
  <div class="page-container">
        ...
        <form @submit.prevent="save">
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input v-model="form.name" placeholder="VD: Đỏ, Xanh" required class="input-field" />
                </div>
                <div class="form-group">
                    <label>Chọn mã màu</label>
                    <input v-model="form.hex" type="color" class="input-field" style="height: 46px; padding: 5px; cursor: pointer;" />
                </div>
            </div>
    ...
    <div v-if="deleteConfirmId" class="modal-overlay" @click.self="deleteConfirmId = null">
      <div class="confirm-dialog">
        ...
        <h3>Xác nhận xóa</h3>
        <p>Bạn có chắc chắn muốn xóa Màu này? Hành động này không thể hoàn tác.</p>
...
export default {
  ...
  methods: {
    ...
    async executeDelete() {
      ...
      try {
        await ColorsService.delete(this.deleteConfirmId);
        await this.loadData();
